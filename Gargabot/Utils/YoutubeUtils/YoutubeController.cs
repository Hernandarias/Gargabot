...
namespace Gargabot.Utils.Youtube
{
    public static class YoutubeController
    {
        ...
        public static async Task<List<YoutubeVideo>> getVideosFromPlaylist(string url)
        {
            ...
            try
            {
                YoutubeClient youtube = _youtube;
        ...
        public static async Task<string> getAudioRealUrl(string url)
        {
            ...
            try
            {
                streamInfo = (AudioOnlyStreamInfo)streamManifest.GetAudioOnlyStreams().GetWithHighestBitrate();
            }
            catch
            {
                streamInfo = streamManifest.GetAudioOnlyStreams().OrderByDescending(x => x.Bitrate).Skip(1).FirstOrDefault()!;
            }
