        ...
        public static async Task<string> getYoutubeMusicUrlFromCompleteTrackName(string search, bool secondTry)
        {
            ...
            try
            {
                ...
                if (videoIdMatch.Success)
                {
                    ...
                    if (await YoutubeUtils.checkIfTitlesMatch(search.Substring(0, search.IndexOf("¡")-1), url))
                    {
                        return url;
                    }
                }
                ...
                else
                {
                    ...
                    {
                        string auxUrl = await YoutubeUtils.checkIfPlaylistHasTrack(search.Substring(0, search.IndexOf("¡") - 1), buildPlaylistUrl(playlistMatch.Groups[1].Value.Trim()));   
                        if (auxUrl != "")
                        {
                            return auxUrl;
                        }
        ...
        public static async Task<string> getYoutubeMusicUrlFromQuery(string search)
        {
            try
            {
                ...
                if (trackIdMatch.Success)
                {
                    ...
                    if(await YoutubeUtils.checkIfVideoIsAutogeneratedByYoutube(buildUrl(trackIdMatch.Groups[2].Value.Trim())))
                    {
